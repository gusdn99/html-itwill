 ...
 * 파라미터를 선언할 때는 const, let, var 키워드를 사용하지 X
 */
...
// JS 함수는 파라미터의 타입을 검사하지 X
result = add('Hello', 'JavaScript');
console.log(`result = ${result}`);

// JS 함수는 파라미터의 개수를 검사하지 X
...
/*
 * JS 함수는 객체(object)!!
 * 1. 프로퍼티(property)를 가질 수 O (예) arguments
 * 2. 변수에 저장할 수 O
 * 3. 다른 함수를 호출할 때 아규먼트로 함수 (객체)를 전달할 수 O
 * 4. 함수 내부에서 다른 함수를 선언(정의)할 수 O
 * 5. 함수 (객체)를 리턴할 수 O
 */

const plus = add; // 함수 객체 add를 변수 plus에 할당(저장); add에 괄호 없음.
...
console.log(calculate(1, 2, function (x, y) {return x - y;} )); // -1
console.log(calculate(1, 2, (x, y) => x / y)); // 0.5
// -> 이벤트 리스너(핸들러) 설정할 때 많이 사용되는 코드 패턴.
// 콜백(callback): (나중에 호출하기 위해서) 아규먼트로 전달하는 함수.

// 지역(내부) 함수: 함수 안에서 선언하는 함수.
function increase(n) {
    // 지역 함수는 바깥 함수의 지역 변수(파라미터 포함)들을 사용할 수 O
    function addN(x) {
        return x + n;
    }
