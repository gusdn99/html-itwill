...
public class ForwardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// - 같은 WAS의(톰캣이 같은) 같은 웹 애플리케이션(서버의 프로젝트가 같은) 안에서만 페이지를 이동하는 방식.
		// - 최초 요청 주소가 바뀌지 않음. URL 주소가 'example.jsp'로 바뀌지 X
		// - request, response 객체가 유지됨. servlet의 request 객체와 jsp의 request 객체가 같음. (response도 마찬가지) 
		// - 다른 WAS 또는 다른 웹 애플리케이션의 페이지로는 포워드(forward)할 수 X
	}
