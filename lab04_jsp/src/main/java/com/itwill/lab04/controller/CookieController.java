...
public class CookieController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// count 변수의 값을 request 객체의 속성(attribute)으로 추가 --> JSP(뷰)로 전달할 수 O
		request.setAttribute("visitCount", count); // 쿠키 요청에는 count 1
		
		count++; // 방문 횟수를 증가
		// 방문 횟수를 저장한 쿠키를 response 객체에 포함.
		Cookie visitCookie = new Cookie("cnt", String.valueOf(count)); // int 타입을 String 타입으로 변환
		
		visitCookie.setMaxAge(24 * 60 * 60); // 쿠키 만료 기간 설정. 단위: 초(second).
		// 쿠키의 만료기간(maxAge)를 설정하지 않으면, 브라우저가 닫힐 때 쿠키는 만료됨.
		response.addCookie(visitCookie); // 쿠키 응답에는 count 2
		
		// 뷰로 요청을 전달.
		request.getRequestDispatcher("/WEB-INF/views/cookie.jsp").forward(request, response);
		// => http://localhost:8080/lab04//WEB-INF/views/cookie.jsp => "WEB-INF" 폴더는 브라우저가 접근할 수 없는 폴더
	}
