...
<body>
    ...
    <main>
        ...
        JSP action tag는 대/소문자를 구분 O (HTML 태그는 대/소문자 구분하지 X)
        ...
        <%--
        setProperty에서 property에는 contact 클래스의 setter 메서드 이름, name에는 useBean의 id(50번째 줄) 
        --%>
        ...
        <p>
        ID: <jsp:getProperty property="id" name="contact2"/> <br /> <%-- contact2.getId(); --%>
        <%--
        getProperty에서 property에는 contact 클래스의 getter 메서드 이름, name에는 useBean의 id(50번째 줄)
        --%>
    ...
    <%--
    <%@ include file="" %>와 비슷하지만, JSP 파일들이 각각 컴파일된 후 합쳐짐.(.jspf는 사용하지 X)
    <jsp:include page="header.jspf"></jsp:include> => 이상하게 나옴.
    --%>
</body>
</html>